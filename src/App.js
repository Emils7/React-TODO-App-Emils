...
// Funkcijas
  ...
  // Funkcijas paredzētas pievienoto uzdevumu saglabāšanai pēc pārlādes.
  useEffect(() => {
    // Tiek aktivizēts, kad tiek pievienots jauns komponents
  ...
  useEffect(() => {
    // Tiek aktivizēts, kad tiek atjaunināts todoObjects masīvs
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(todoObjects));
  }, [todoObjects]);

  useEffect(() => {
    // Tiek aktivizēts, lai nofiltrētu izpildītus, neizpildītus un visus uzdevumus.
    const filterHandler = () => {
      switch (filter) {
        case 'completed':
  ...
  function addTodo(todo) {
    // pievieno jaunu uzdevumu todoObjects masīva sākumam
    setTodoObjects([todo, ...todoObjects]);
  }

  // dzēš uzdevumu no todoObjects masīva
  function deleteTodo(id) {
    setTodoObjects(todoObjects.filter(todo => todo.id !== id));
  }

// Aplikācijas struktūras atgriešana
  return (
    <div className="todoContainer">
      <header className="App-header">
        <h1 className='titleStyle'>TO DO Saraksts</h1>
        <TodoForm
          addTodo={addTodo}
          setFilter={setFilter}
        />
